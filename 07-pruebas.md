...
### [2026-04-05] — Instalación de Proxmox en VirtualBox (laptop Ubuntu — usuario luna)

- **Equipo/Entorno:** Laptop con Ubuntu, VirtualBox, usuario `luna` / root
...
```
Causa: las VMs están registradas bajo el usuario `luna`, no bajo root.

Solución:
```bash
su - luna -c "VBoxManage list vms"
# Resultado: "proxmox" {e3a7bed9-83fd-49e5-8bc7-75eed7d0bddf}

su - luna -c "VBoxManage modifyvm 'proxmox' --nested-hw-virt on"

su - luna -c "VBoxManage showvminfo proxmox | grep -i nested"
# Resultado: Nested VT-x/AMD-V: enabled ✅
```

**Resultado — Proxmox instalado correctamente**

- IP asignada: `192.168.0.18`
- Versión: Proxmox VE 9.1.1
- Nodo: `pve`
- Almacenamiento: `local (pve)` + `local-zfs (pve)`
- Acceso web: `https://192.168.0.18:8006` ✅
