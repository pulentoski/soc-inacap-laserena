# Proyecto SOC Académico — INACAP La Serena
...
- Reproducir la instalación completa siguiendo esta guía.
...
1. Instalar y configurar Proxmox VE como hipervisor de Tipo 1 en servidores físicos de rack.
2. Desplegar Wazuh como plataforma SIEM/XDR para la recolección y análisis de eventos de seguridad.
...
### Actividades de ataque y defensa (Red Team / Blue Team)
...
### Actividades de administración y mantención
...
| **Wazuh Indexer** | Base de datos basada en OpenSearch donde se almacenan y indexan todos los eventos para permitir búsquedas rápidas. |
...
|---|---|---|
| **DVWA** (Damn Vulnerable Web Application) | Aplicación PHP con vulnerabilidades web clásicas: SQL Injection, XSS, File Upload, etc. Permite ajustar el nivel de dificultad. | 8080 |
...
VirtualBox es un hipervisor de Tipo 2 (hosted): se instala sobre un sistema operativo existente (como Windows) y permite crear máquinas virtuales dentro de él. En este proyecto se usa exclusivamente como **entorno de práctica previo**, para que los alumnos instalen y conozcan Proxmox VE en su propia laptop antes de configurar los servidores reales del rack.
...
```
                ...
                ┌─────────────────┼─────────────────┐
                │                                   │
         VLAN 10 — Gestión                  VLAN 20 — Laboratorio
                │                                   │
    ┌───────────┴───────────┐          ┌────────────┴────────────┐
...
> Este repositorio se construye de forma incremental. A medida que se avanza en la instalación y configuración de los equipos reales en la sala A315, la documentación se actualiza con los hallazgos reales: problemas encontrados, soluciones aplicadas y ajustes a la arquitectura original.
>
